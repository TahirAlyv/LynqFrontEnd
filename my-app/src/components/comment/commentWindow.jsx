...
export default function CommentWindow({
    ...
    const connect = async () => {
      try {
        const connection = new signalR.HubConnectionBuilder()
          .withUrl("https://localhost:7257/commenthub", {
        ...
        showToast?.("Comment connection hazır deyil.", "error");
      }
    };
  ...
  const sendComment = async (text) => {
    ...
    if (!trimmed) {
      showToast?.("Comment boş ola bilməz.", "error");
      return;
    }
    ...
    try {
      ...
      showToast?.("Comment əlavə edildi.", "success");
    } catch (error) {
      console.error("Send comment failed:", error);
      showToast?.("Comment əlavə olunmadı.", "error");
    }
  };
  ...
  const handleDelete = async (commentId) => {
    try {
      ...
      showToast?.("Comment silindi.", "success");
    } catch (err) {
      console.error("Delete failed:", err);
      showToast?.("Comment silinmədi.", "error");
    }
  };
  ...
  const handleUpdate = async ({ commentId, text }) => {
    ...
    try {
      ...
      showToast?.("Comment yeniləndi.", "success");
    } catch (err) {
      console.error("Update failed:", err.response?.data || err);
      showToast?.("Comment yenilənmədi.", "error");
    }
  };
